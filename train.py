...
tf.app.flags.DEFINE_string('source_train_data', 'data/fr_mix_de_mix/europarl-v7.1.4M.de', 'Path to source training data')
tf.app.flags.DEFINE_string('target_train_data', 'data/fr_mix_de_mix/europarl-v7.1.4M.fr', 'Path to target training data')
...
tf.app.flags.DEFINE_string('attention_type', 'luong', 'Attention mechanism: (bahdanau, luong)')
...
tf.app.flags.DEFINE_integer('max_decode_step', 100, 'Maximum time step limit to decode')
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=FLAGS.allow_soft_placement, 
        ...
        try:
            ...
            for idx, source_seq in enumerate(test_set):
                ...
                # predicted_ids: GreedyDecoder; [batch_size, max_time_step]
                #                BeamSearchDecoder; [batch_size, max_time_step, beam_width]
                predicted_ids = model.predict(sess, encoder_inputs=source, 
                                              encoder_inputs_length=source_len)
                                  
                for seq in predicted_ids:
                    #print(data_utils.seqs2words(seq, target_inverse_dict))
                    fout.write(str(data_utils.seqs2words(seq, target_inverse_dict)) + '\n')
                if idx % 100 == 0:
